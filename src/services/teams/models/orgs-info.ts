...
export const orgsInfo = z.lazy(() => {
  return z.object({
    _orgId_: z.array(teamRoles).optional(),
  });
});
 ...
 * @property {TeamRoles[]} - A list of the organization's roles. The array's name is the organization's ID.
 */
...
export const orgsInfoResponse = z.lazy(() => {
  return z
    .object({
      '{{orgId}}': z.array(teamRoles).optional(),
    })
    .transform((data) => ({
      _orgId_: data['{{orgId}}'],
    }));
