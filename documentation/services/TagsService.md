...
(async () => {
  ...
  const data = await postmanApi.tags.getCollectionTags(
...
```typescript
import { AscDescDefaultDesc, PostmanApi, TagsEntityType } from '@postman/api-sdk';
...
(async () => {
  ...
  const data = await postmanApi.tags.getTaggedEntities('needs-review', {
    limit: 2,
...
(async () => {
  ...
  const data = await postmanApi.tags.getWorkspaceTags('1f0df51a-8658-4ee8-a2a1-d2567dfa09a9');
